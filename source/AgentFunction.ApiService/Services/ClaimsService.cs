...
{
    Task<IEnumerable<Claim>> GetCustomerClaimsHistoryAsync(string customerId);
}
...
public class ClaimsService : IClaimsService
{
    ...
    public Task<IEnumerable<Claim>> GetCustomerClaimsHistoryAsync(string customerId)
    {
        ...
        for (int i = 0; i < claimsCount; i++)
        {
            ...
            var claim = new Claim(
                ...
                ClaimantName: GenerateRandomName(random),
                ClaimantContact: GenerateRandomEmail(random),
                ...
                LicensePlate: GenerateRandomLicensePlate(random),
        ...
        return Task.FromResult(claims.OrderByDescending(c => c.DateOfAccident).AsEnumerable());
    }
    ...
    private static string GenerateRandomEmail(Random random)
    {
        ...
        var name = GenerateRandomName(random).Replace(" ", ".").ToLower();
        return $"{name}@{domains[random.Next(domains.Length)]}";
    }

    private static string GenerateRandomLicensePlate(Random random)
    {
