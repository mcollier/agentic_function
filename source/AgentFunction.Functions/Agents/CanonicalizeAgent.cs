...
public sealed class CanonicalizeAgent(Kernel kernel, ILogger<CanonicalizeAgent> logger) :
                     AgentBase<FnolClaim, CanonicalClaim>(
                        kernel,
                        logger,
...
{
    private readonly ILogger<CanonicalizeAgent> _typedLogger = logger ?? throw new ArgumentNullException(nameof(logger));
