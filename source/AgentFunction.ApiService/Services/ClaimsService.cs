...
public class ClaimsService : IClaimsService
{
    ...
    private static readonly string[] Statuses = ["Submitted", "UnderReview", "Approved", "Rejected", "Closed"];
    ...
    public Task<IEnumerable<Claim>> GetClaimsHistoryAsync(string customerId)
    {
        ...
        for (int i = 0; i < claimsCount; i++)
        {
            ...
            var claim = new Claim
            {
                ...
                Status = Enum.Parse<ClaimStatus>(Statuses[random.Next(Statuses.Length)])
            };
